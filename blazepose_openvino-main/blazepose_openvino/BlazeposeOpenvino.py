...
class BlazeposeOpenvino:
    ...
    def lm_render(self, frame, region):
        if region.lm_score > self.lm_score_threshold:
            ...
            if self.use_gesture and self.show_gesture:
                cv2.putText(frame, region.gesture, (region.landmarks_padded[12,0]-10, region.landmarks_padded[12,1]+10), 
                        cv2.FONT_HERSHEY_PLAIN, 5, (0,1190,255), 3)
                for i in range(self.frame_size):
                    if(i%100 == 0):
                        cv2.line(frame, (i, 0), (i, self.frame_size), (255, 0, 0), 1, 1)
                        cv2.line(frame, (0, i), (self.frame_size, i), (255, 0, 0), 1, 1)
    ...
    def recognize_gesture(self, r):           
        ...
        shoulders_angle = angle_with_y(r.landmarks_abs[11,:2] - r.landmarks_abs[12,:2])
        # right_pose = int((shoulders_angle +202.5) / 45) % 8
        r.gesture = str(shoulders_angle)
